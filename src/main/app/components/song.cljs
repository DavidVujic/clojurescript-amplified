...
(defn- fn-song-card []
  (let [theme          (styles/use-theme)
        right-to-left? (= (:direction theme) "rtl")]
    [card {:sx {:display "flex"}}
     [box {:sx {:display "flex" :flex-direction "column"}}
      ...
      [box {:sx {:display "flex" :align-items "center" :pl 1 :pb 1}}
       [icon-button {:aria-label "previous"}
        (if right-to-left? [skip-next] [skip-previous])]
       [icon-button {:aria-label "play/pause"} [play-arrow]]
       [icon-button {:aria-label "next"}
        (if right-to-left? [skip-previous] [skip-next])]]]
